...
<body>
	<%
		//購物車功能
		//1:首先考慮我購買的是哪一本書籍，這裏可以使用id確認也可以使用session中取出哪一本書籍
		...
		//3:考慮如何把書籍放到購物車中
		//3.1:第一考慮購物車中是否有該書籍，所以先從購物車中獲取該書籍，如果為空，那麽沒有該書籍
		CartItem item = (CartItem) cart.get(stationery.getStationeryId());
		if (item == null) {
			//如果購物車中不存在該書籍，那麽創建，且數量默認為1
			item = new CartItem();
			//將書籍放到購物車中
		...
		//4:考慮如何把購物車項(即挑選的書籍是哪一個和書本的數量)放到購物車中
